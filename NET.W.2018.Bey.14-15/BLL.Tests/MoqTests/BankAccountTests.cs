...
namespace BLL.Tests.MoqTests
{
    ...
    public class BankAccountTests
    {
        ...
        {
            var accountService = new AccountService(_accountRepository.Object, _bonusCounter.Object);
        ...
        public void CloseAccount_Test(string firstName, string lastName, AccountType type)
        {
            ...
            _accountRepository.Setup(n => n.Update(It.IsAny<BankAccount>())).Returns(new BankAccount(_bonusCounter.Object) { Id = 1, FirstName = firstName, Lastname = lastName, Type = (int)type, IsClosed = true });
            _accountRepository.Setup(n => n.Get(It.IsAny<int>())).Returns(new BankAccount(_bonusCounter.Object) { Id = 1, FirstName = firstName, Lastname = lastName, Type = (int)type, IsClosed = false });
        ...
        public void WithDraw_Test(int number, string firstName, string lastName, AccountType type, decimal amount, int bonus)
        {
            ...
            _accountRepository.Setup(n => n.Get(It.IsAny<int>())).Returns(new BankAccount(_bonusCounter.Object) { Id = number, FirstName = firstName, Lastname = lastName, Type = (int)type, Amount = amount, Bonus = bonus, IsClosed = false });
            _accountRepository.Setup(n => n.Update(It.IsAny<BankAccount>())).Returns(new BankAccount(_bonusCounter.Object) { Id = number, FirstName = firstName, Lastname = lastName, Type = (int)type, Amount = amount - 10, Bonus = bonus, IsClosed = false });
